...
public class ItemStackBuilder {
    ...
    public boolean softCompare(@Nonnull ItemLazyWrapper itemLazyWrapper) {
        if (this.material != itemLazyWrapper.getItemStack().getType()) {
            return false;
        }

        if (this.amount != null) {
            if (this.amount != itemLazyWrapper.getItemStack().getAmount()) {
                return false;
            }
        }

        if (this.itemMetaBuilder != null) {
            if (!itemLazyWrapper.hasItemMeta() || !this.itemMetaBuilder.softCompare(itemLazyWrapper.getItemMeta())) {
                return false;
            }
        }
